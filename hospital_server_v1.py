            ...
            for row in rows:
                ...
                if row["override_action"] == "AMBULANCE":
                    ...
                    if not amb_orig or not dispatch_ts:
                        continue
                    # Total round-trip: ambulance TO patient + 5 min pickup + return trip
                    total_eta = 2 * amb_orig + 5
                    try:
                        ...
                        new_eta = max(0, round(total_eta - elapsed_min))
                    except Exception:
                        new_eta = max(0, round((row["eta_minutes"] or 0) - TICK / 60.0))
                    if new_eta == 0:
                        # Ambulance has reached patient — transition directly to
                        # in_treatment so the dashboard moves them to Under Treatment.
                        conn.execute(
                            "UPDATE patient_queue SET eta_minutes=0, status='in_treatment', "
                            "arrival_time=?, treatment_started_at=?, updated_at=? WHERE patient_id=?",
                            (now_iso, now_iso, now_iso, pid)
                        )
                        logger.info("Patient %s ambulance arrived → status='in_treatment'.", pid)
                    else:
                        conn.execute(
...
_LANG_GUIDES = {
    ...
    "German": (
        ...
        "- For pain CHARACTER questions: use 'Wie würden Sie den Schmerz beschreiben?' — NEVER 'Welche Farbe hat der Schmerz?' ('Farbe' means color, not character).\n"
        "  Correct pain character options: 'Drückend', 'Stechend', 'Brennend', 'Pulsierend', 'Dumpf', 'Keines davon'\n\n"
        "EXAMPLE:\n"
        "Input:  {\"q\": \"Does the pain radiate to your arm or jaw?\", \"opts\": [\"Yes\", \"No\"]}\n"
...
def _enrich_patient(p: dict) -> dict:
    ...
    if (p.get("complaint_en") and
        ...
        # Strategy 2: GPT fallback (always available when OPENAI_API_KEY is set)
...
def patient_submit(body: SubmitRequest):
    ...
    # Store ambulance REQUEST (pending staff approval) — movement thread only fires on 'AMBULANCE'
    if body.ambulance_note and body.ambulance_note.strip():
        eta_to_pt = int(body.ambulance_to_patient_eta) if body.ambulance_to_patient_eta and body.ambulance_to_patient_eta > 0 else 10
        hq.setup_ambulance_request(
            record["patient_id"],
...
def api_patient_status(patient_id: str):
    ...
    return {
        ...
        "amb_eta_patient": match.get("amb_eta_patient") or None,
        "amb_dispatch_at": match.get("amb_dispatch_at") or None,
    }


@app.patch("/api/patient/{patient_id}/ambulance/request")
def api_ambulance_request(patient_id: str, body: dict):
    """Patient app posts ambulance request after already being registered.
    Called when submitToBackend() already ran (S._submitted=True) but ambulance
    choice was made afterwards — avoids double-submit guard.
    """
    note = body.get("ambulance_note") or ""
    eta_to_patient = int(body.get("ambulance_to_patient_eta") or 10)
    if not note.strip():
        raise HTTPException(400, "ambulance_note required")
    all_p = hq.get_all_patients(limit=200)
    match = next((p for p in all_p if p["patient_id"] == patient_id), None)
    if not match:
        raise HTTPException(404, "Patient not found")
    # If already dispatched, don't overwrite
    if match.get("override_action") == "AMBULANCE":
        return {"ok": True, "already_dispatched": True}
    ok = hq.setup_ambulance_request(patient_id, note.strip(), eta_to_patient)
    if not ok:
        raise HTTPException(500, "Failed to save ambulance request")
    logger.info("Ambulance request saved for patient %s (ETA %s min)", patient_id, eta_to_patient)
    return {"ok": True}


@app.post("/api/patient/{patient_id}/ambulance/dispatch")
def api_ambulance_dispatch(patient_id: str):
    """Staff approval: dispatch ambulance for a pending AMBULANCE_REQUEST patient."""
    all_p = hq.get_all_patients(limit=200)
    match = next((p for p in all_p if p["patient_id"] == patient_id), None)
    if not match:
        raise HTTPException(404, "Patient not found")
    if match.get("override_action") not in ("AMBULANCE_REQUEST", "AMBULANCE"):
        raise HTTPException(400, "Patient has no pending ambulance request")
    if match.get("override_action") == "AMBULANCE":
        return {"ok": True, "already_dispatched": True}
    ok = hq.approve_ambulance_dispatch(patient_id)
    if not ok:
        raise HTTPException(500, "Failed to dispatch ambulance")
    logger.info("Ambulance dispatched by staff for patient %s", patient_id)
    return {"ok": True, "dispatched_at": datetime.now(timezone.utc).isoformat()}


@app.patch("/api/patient/{patient_id}/doctor")
def api_assign_doctor(patient_id: str, body: dict):
    """Persist doctor assignment so patient app can be notified."""
    doctor_name = body.get("doctor_name") or ""
    doctor_id = body.get("doctor_id") or ""
    assigned_at = body.get("assigned_at") or ""
    try:
        import sqlite3 as _sq2, datetime as _dt2
        server_ts = _dt2.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
        # Prefer the client-supplied timestamp but fall back to server time
        record_at = assigned_at or server_ts
        conn2 = _sq2.connect(str(hq.db_path))
        rows_affected = conn2.execute(
            "UPDATE patient_queue SET assigned_doctor=?, doctor_assigned_at=?, updated_at=? WHERE patient_id=?",
            (f"{doctor_name}||{doctor_id}", record_at, server_ts, patient_id),
        ).rowcount
        conn2.commit()
        conn2.close()
        if rows_affected == 0:
            logger.warning("Doctor assign: patient %s not found in DB", patient_id)
            raise HTTPException(404, "Patient not found")
        logger.info("Doctor %s assigned to patient %s", doctor_name, patient_id)
        return {"ok": True, "assigned_at": record_at}
    except HTTPException:
        raise
    except Exception as exc:
        logger.warning("Doctor assignment persist failed: %s", exc)
...
def api_patient_reply(patient_id: str, body: dict):
    ...
    all_p = hq.get_all_patients(limit=200)
    match2 = next((p for p in all_p if p["patient_id"] == patient_id), None)
    if not match2:
        raise HTTPException(404, "Patient not found")
    ...
    try:
        conn3 = _sq3.connect(str(hq.db_path))
        row = conn3.execute("SELECT patient_messages FROM patient_queue WHERE patient_id=?", (patient_id,)).fetchone()
        existing = []
        if row and row[0]:
            try:
                existing = _json2.loads(row[0])
            except Exception:
                existing = []
        existing.append(entry)
        conn3.execute(
            "UPDATE patient_queue SET patient_messages=? WHERE patient_id=?",
            (_json2.dumps(existing, ensure_ascii=False), patient_id),
        )
        conn3.commit()
        conn3.close()
    except Exception as exc:
        logger.warning("Patient reply persist failed: %s", exc)
        raise HTTPException(500, "DB error")
