    ...
    <div class="hn-item" onclick="selectHn('${p.id}','${escHtml(p.name)}')">
      ...
      <div><div class="hn-item-name">${escHtml(p.name)}</div><div class="hn-item-detail">${escHtml(p.detail)}</div></div>
        ...
        async function processAudio() {
            // Wait briefly for Web Speech final result (fires after mediaRec.stop)
            await new Promise(r => setTimeout(r, 350));

            const wsr = (webSpeechResult || '').trim();
            const blob = new Blob(audioChunks, { type: 'audio/webm;codecs=opus' });

            // If blob is too small but Web Speech has a result, use it immediately
            if (blob.size < 500) {
                if (wsr) { setComplaintFromAudio(wsr, S.selectedLang || S.detectedLang); webSpeechResult = ''; return; }
                const m = { en: 'Recording too short — please try again.', de: 'Aufnahme zu kurz — erneut versuchen.', tr: 'Kayıt çok kısa — tekrar deneyin.' };
                toast(m[S.lang] || m.en, 'warn'); webSpeechResult = ''; return;
            }

            // If Web Speech already got a good result (≥ 3 words), use it directly
            // and send audio to server in background for quality check only
            if (wsr && wsr.split(' ').length >= 3) {
                setComplaintFromAudio(wsr, S.selectedLang || S.detectedLang);
                webSpeechResult = '';
                // Background: try server transcription and upgrade if better
                try {
                    ...
                    if (d.text && d.text.trim() && d.text.trim().split(' ').length > wsr.split(' ').length) {
        ...
        function setComplaintFromAudio(text, langCode) {
            ...
            if (transcriptBox) transcriptBox.classList.add('visible');
